...
def actions_add(bits, CC_DEP1, CC_DEP2, CC_NDEP):
    ...
    pf = 0
    af = 0
    argL = int(CC_DEP1 & 4294967295)
    argR = int(CC_DEP2 & 4294967295)
    res = int((argL + argR) & 4294967295)
...
def actions_adc(bits, CC_DEP1, CC_DEP2, CC_NDEP):
    ...
    pf = 0
    af = 0
    oldC = int(CC_NDEP & X86G_CC_MASK_C & 4294967295)
    argL = int(CC_DEP1 & 4294967295)
    argR = int((CC_DEP2 ^ oldC) & 4294967295)
    res = int((argL + argR + oldC) & 4294967295)
    res_new = getRes_New(bits, res)
    argL_new = getRes_New(bits, argL)
    if oldC == 0:
        if res_new < argL_new:
...
def actions_inc(bits, CC_DEP1, CC_DEP2, CC_NDEP):
    ...
    pf = 0
    af = 0
    res = int(CC_DEP1 & 4294967295)
    argL = int((res - 1) & 4294967295)
    argR = 1
    res_new = getRes_New(bits, res)
    cf = int(CC_NDEP & X86G_CC_MASK_C & 4294967295)
    pf = parity_table[int(res & 255)]
    af = (res ^ argL ^ argR) & 0x10
    if res_new == 0:
        zf = 1 << 6
...
def actions_shl(bits, CC_DEP1, CC_DEP2, CC_NDEP):
    ...
    argR = 1
    CC_DEP1_new = getRes_New(bits, CC_DEP1)
    cf = int((CC_NDEP >> (bits - 1)) & X86G_CC_MASK_C & 4294967295)
    pf = parity_table[int(CC_DEP1 & 255)]
    af = 0
    if CC_DEP1_new == 0:
        zf = 1 << 6
...
def x86g_calculate_eflags_all_WRK(cc_op, cc_dep1_formal, cc_dep2_formal, cc_ndep_formal):
    ...
    if op_str == "X86G_CC_OP_COPY":
        ...
        raise Exception("other operation in x86g_calculate_condition")
...
def x86g_calculate_condition(cond, cc_op, cc_dep1, cc_dep2, cc_ndep):
    ...
    if con_str == "X86CondO" or con_str == "X86CondNO":
        of = eflags >> X86G_CC_SHIFT_O
        return 1 & (inv ^ of)
    elif con_str == "X86CondNZ" or con_str == "X86CondZ":
        zf = eflags >> X86G_CC_SHIFT_Z
        return 1 & (inv ^ zf)
    elif con_str == "X86CondNB" or con_str == "X86CondB":
        cf = eflags >> X86G_CC_SHIFT_C
        return 1 & (inv ^ cf)
    elif con_str == "X86CondNBE" or con_str == "X86CondBE":
        cf = eflags >> X86G_CC_SHIFT_C
        zf = eflags >> X86G_CC_SHIFT_Z
        return 1 & (inv ^ (cf | zf))
    elif con_str == "X86CondNS" or con_str == "X86CondS":
        sf = eflags >> X86G_CC_SHIFT_S
        return 1 & (inv ^ sf)
    elif con_str == "X86CondNP" or con_str == "X86CondP":
        pf = eflags >> X86G_CC_SHIFT_P
        return 1 & (inv ^ pf)
    elif con_str == "X86CondNL" or con_str == "X86CondL":
        sf = eflags >> X86G_CC_SHIFT_S
        of = eflags >> X86G_CC_SHIFT_O
        return 1 & (inv ^ (sf ^ of))
    elif con_str == "X86CondNLE" or con_str == "X86CondLE":
        sf = eflags >> X86G_CC_SHIFT_S
        of = eflags >> X86G_CC_SHIFT_O
        zf = eflags >> X86G_CC_SHIFT_Z
        return 1 & (inv ^ ((sf ^ of) | zf))
    else:
        raise Exception("x86_calculate_condition exception")
...
def x86g_calculate_eflags_c(cc_op, cc_dep1, cc_dep2, cc_ndep):
    op_str = X86CCop[cc_op]
    if op_str in ["X86G_CC_OP_LOGICL", "X86G_CC_OP_LOGICW", "X86G_CC_OP_LOGICB"]:
        return 0
    elif op_str == "X86G_CC_OP_SUBL":
        cc_dep1_new = int(cc_dep1 & 0xFFFFFFFF)
        cc_dep2_new = int(cc_dep2 & 0xFFFFFFFF)
        if cc_dep1_new < cc_dep2_new:
            return X86G_CC_MASK_C
        else:
            return 0
    elif op_str == "X86G_CC_OP_SUBW":
        value1 = int((cc_dep1 & 0xFFFF) & 0xFFFFFFFF)
        value2 = int((cc_dep2 & 0xFFFF) & 0xFFFFFFFF)
        if value1 < value2:
            return X86G_CC_MASK_C
        else:
            return 0
    elif op_str == "X86G_CC_OP_SUBB":
        value1 = int((cc_dep1 & 0xFF) & 0xFFFFFFFF)
        value2 = int((cc_dep2 & 0xFF) & 0xFFFFFFFF)
        if value1 < value2:
            return X86G_CC_MASK_C
        else:
            return 0
    elif op_str in ["X86G_CC_OP_INCL", "X86G_CC_OP_DECL"]:
